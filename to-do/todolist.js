...
function adddata() {
    ...
    data.push({ title, description, status, priority , id:uid()})
    render(data)
}
function render(data) {
    ...
    data.map((el) => {
        const carddata = `
        <div class="cardtext bg-white flex gap-12 justify-between" ${el.id}>
        ...
        <div>
            <div class= "remove icon flex justify-center align-center margin-top-5 grey-hover" ${el.remove}>
                <div>
                    <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="100" height="100" viewBox="0 0 30 30">
